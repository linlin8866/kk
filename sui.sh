...
# 纯离线：跳过所有联网依赖安装
install_base() {
    echo -e "${yellow}Offline mode enabled, skip all online operations${plain}"
}
...
config_after_install() {
    ...
    if [[ "${config_confirm}" == "y" || "${config_confirm}" == "Y" ]]; then
        ...
        read config_path
        read -p "Enter the ${yellow}subscription port${plain} (leave blank for existing/default value):"
        read config_subPort
        read -p "Enter the ${yellow}subscription path${plain} (leave blank for existing/default value):"
        read config_subPath
...
install_s-ui() {
    ...
    echo -e "${green}s-ui local offline installation finished, running now...${plain}"
...
echo -e "${green}Executing offline install...${plain}"
install_base
install_s-ui
